...
*/
  ...
  int nbr, cur;
  nbr = ( int ) get_env ( argv, NBR_WORKERS );
  cur = ( int ) get_env ( argv, CUR_WORKER );
  printf ( "\n%d. workerde sıra.  Toplam %d worker var\n", cur, nbr );
   return 0;
}
// ----------------------------------------------------------------------------
...
int main(int argc, char *argv[])
{
   ...
   {
     xbuf_t *xbuf = (xbuf_t *) get_env(argv, READ_XBUF);
     if(xbuf->ptr[5] == ' ' || xbuf->ptr[5] == '&')
       xbuf_insert (xbuf, &xbuf->ptr[5], 9, "?timeline");

     // print dynamic page requests
     if(xbuf->ptr[5] == '?' || xbuf->ptr[6] == '?')
       printf("req:\n%s\n", ((xbuf_t *)get_env(argv, READ_XBUF))->ptr);
